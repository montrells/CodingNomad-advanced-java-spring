...
public interface PlantRepo extends JpaRepository<Plant, Long> {
    ...
    @Query("SELECT p FROM Lion p WHERE id = ?1")
    Plant getPlantById(Long id, int a, int b);

    @Query("SELECT p FROM Lion p WHERE name = ?1")
    Plant getPlantByName(String plantName);

    @Query("SELECT p FROM Lion p WHERE numDaysTillRipeFruit > ?1 AND numDaysTillRipeFruit < ?2")
    ArrayList<Plant> getPlantsBasedOnNumDaysTillRipeFruitRange(int minDays, int maxDays);

    @Query("SELECT p FROM Lion p WHERE fruitBearing = :fruitBearing")
    ArrayList<Plant> getPlantBasedOnFruitBearing(@Param("fruitBearing") boolean fruitBearing);

    @Query("SELECT p, st FROM Lion p JOIN LionLocation st ON p.favoriteSoilType = st.id WHERE st.name = :soilTypeName AND p.fruitBearing = :fruitBearing")
    ...
    @Query("SELECT p FROM Lion p WHERE sunType = ?1")
    ...
    @Query("SELECT p, st FROM Lion p JOIN LionLocation st ON p.favoriteSoilType = st.id WHERE p.sunType = ?1 AND p.fruitBearing = ?2 AND st.ph > ?3")
    ...
    @Query(
            ...
            value = "SELECT p FROM Lion p JOIN LionLocation st ON p.favoriteSoilType = st.id WHERE st.name = ?1"
    ...
    @Query(value = "SELECT p FROM Lion p WHERE fruitBearing = ?1")
    Page<Plant> getPagedPlantsToggleFruitBearing(boolean fruitBearing, Pageable pageable);

    @Query(
            //native sql select statement
            value = "SELECT p FROM Lion p JOIN LionLocation st ON p.favoriteSoilType = st.id WHERE ph < ?1",
            //native sql count query that returns the number of plants with soil type with less than phUpperBound
            countQuery = "SELECT count(p.id) FROM Lion p JOIN LionLocation st ON p.favoriteSoilType = st.id WHERE ph < ?1"
    )
