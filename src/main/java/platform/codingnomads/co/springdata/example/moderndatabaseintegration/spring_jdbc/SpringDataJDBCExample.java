...
public class SpringDataJDBCExample implements CommandLineRunner {
    ...
    public void run(String... strings) {
        ...
        try {
            //create manager table using the JdbcTemplate method "execute"
...
//        for each first & last name pair insert an Manager into the database
        for (Object[] name : splitUpNames) {
            jdbcTemplate.execute(String.format("INSERT INTO managers(first_name, last_name) VALUES ('%s','%s')", name[0], name[1]));
        }

        //query the database for Managers with first name Java
        jdbcTemplate.query(
                        "SELECT id, first_name, last_name FROM managers WHERE first_name = 'Java'",
                        (rs, rowNum) -> new Manager((int) rs.getLong("id"), rs.getString("first_name"), rs.getString("last_name"))
                )
                //print each found manager to the console
